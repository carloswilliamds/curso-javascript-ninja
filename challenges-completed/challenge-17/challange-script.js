...
text.match(/Manuel Marques de Sousa/ig, "Carlos William")
console.log(text.replace(/Manuel Marques de Sousa/ig, "Carlos William"))
console.log(text)
/*
...
console.log(text.replace(/brasileiro/ig, "paulista"))
...
console.log(text.replace(/\w[D-H-d-h]/ig, "0"))
/*
...
let upperCaseText = text.replace(/O Centauro de Luvas/, function(text){
...
const getMonthNumber = (textuser) =>{
    if(typeof textuser === "string"){
        let mesText = textuser.toLowerCase().replace(/\w/, (text) => text.toUpperCase());
        switch (mesText){
            case "Janeiro":
                return "01"
                break;
            ...
            case "Dezembro":
                return "12"
            default:
                    return "Digite um mês valido existente"     
        }
    } else {
        console.log("Digite um mês valido em texto")
    }
}
...
console.log(`O mês ${mesS} é repesentado pelo número ${getMonthNumber(mesS)}`)
console.log(`O mês ${mesD} é repesentado pelo número ${getMonthNumber(mesD)}`)
console.log(`O mês ${mesM} é repesentado pelo número ${getMonthNumber(mesM)}`)
...
let regexDate = /(\d{2}) de (junho|julho) de (\d{4})/ig
...
*/
console.log( '\nReplace de datas:' );
const replaceDate = (regex, dia, mes, ano) =>{
